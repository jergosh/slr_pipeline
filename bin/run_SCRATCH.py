    ...
    for f in glob.glob(path.join(args.indir, "*", "*.fa")):
        for seqr in SeqIO.parse(open(f), 'fasta'):
            ...
            if species_code == "ENSP":
                ...
                input_file = path.join(args.outdir, subdir, basename)
                log_file = path_join(args.outdir, subdir, basename+'.log')

                print input_file
                utils.check_dir(path.join(args.outdir, subdir))
                SeqIO.write([ seqr ], open(input_file, 'w'), 'fasta')
                cmd = cmd_template.format(input_file, path.join(args.outroot, subdir, seqr.id+'_'+basename))
